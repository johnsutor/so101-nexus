...
def test_select_action_calls_model_with_images_in_configured_order_and_float32_state() -> None:
    from so101_nexus_core.policy_adapters import MolmoActPolicy
...
def test_importing_policy_adapters_does_not_require_molmoact_extra(
    ...
    for module_name in list(sys.modules):
        if module_name == "so101_nexus_core.policy_adapters" or module_name.startswith(
            "so101_nexus_core.policy_adapters."
        ):
            monkeypatch.delitem(sys.modules, module_name, raising=False)
    monkeypatch.setitem(sys.modules, "huggingface_hub", None)
    monkeypatch.setitem(sys.modules, "transformers", None)

    module = importlib.import_module("so101_nexus_core.policy_adapters")
