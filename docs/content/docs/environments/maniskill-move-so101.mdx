---
title: ManiSkillMoveSO101-v1
description: Move the TCP in a cardinal direction with the SO-101 arm on the ManiSkill backend.
---

# ManiSkillMoveSO101-v1

Move the robot's end-effector a fixed distance in a specified cardinal direction using the SO-101 robot arm in ManiSkill.

## Overview

| Property | Value |
|----------|-------|
| Environment ID | `ManiSkillMoveSO101-v1` |
| Backend | ManiSkill |
| Robot | SO-101 |
...
env = gym.make("ManiSkillMoveSO101-v1", obs_mode="state", num_envs=1)

# Custom direction
config = MoveConfig(direction="forward", target_distance=0.15)
env = gym.make("ManiSkillMoveSO101-v1", config=config, obs_mode="state", num_envs=1)
```
