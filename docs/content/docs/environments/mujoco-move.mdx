...
Default observation is a 22-dimensional vector (configurable via the `observations` parameter on `MoveConfig`):
...
Reward rises smoothly as the TCP travels toward the target displacement, plus
a one-time bonus on success. Move has just one thing to accomplish, so the
reward reflects the TCP's current displacement, not just a change since last
step. See [Reward Structure](/docs/concepts/tasks-and-success-conditions#reward-structure)
...
Set `ENV_ID = "WarpMove-v1"` in the notebook's environment cell. See [Training with PPO](/docs/training/ppo) for the local CLI workflow.
