...
>
  <source
    src="https://huggingface.co/datasets/johnsutor/MuJoCoPickLift/resolve/main/videos/observation.images.overhead/chunk-000/file-000.mp4"
    type="video/mp4"
  />
  Your browser does not support embedded video. Open the
  <a href="https://huggingface.co/spaces/lerobot/visualize_dataset?path=%2Fjohnsutor%2FMuJoCoPickLift%2Fepisode_0">
...
All five tasks run on the MuJoCo backend on the SO-101 arm. The MuJoCo Warp backend covers all five (Touch, LookAt, Move, PickLift, PickAndPlace) with GPU-parallel simulation, subject to the documented batched-model limitations (fixed per-episode colors, single-cube PickLift and Touch).
