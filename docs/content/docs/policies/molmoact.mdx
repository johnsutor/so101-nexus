...
```bash
pip install "so101-nexus-core[molmoact]"
```
...
from so101_nexus_core import SO101_JOINT_NAMES
from so101_nexus_core.policy_adapters import MolmoActPolicy, RolloutRecorder
from so101_nexus_core.teleop.dataset import FieldSelection, build_features
