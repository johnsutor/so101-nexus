...
## Observation-layout caveat

<Callout type="warn">
The policy has to consume exactly the state layout its demonstrations recorded, so with
`--use-demos true` the script builds both the Warp training env and the MuJoCo evaluator
from the layout the dataset declares, not from the env's current default. When the two
differ (a dataset recorded before a component joined the defaults, for example
`gaze_state`), the run prints the pinned width and the missing components at startup, and
the resulting checkpoint only loads against that layout. Re-record the demos to train on
the current default.
</Callout>
...
Teleop recording is pinned to `pd_joint_pos`, so this relabeling step is the norm rather
than a workaround: joint positions are the only label the leader produces, and every other
action space is recomputed from them. `pd_ee_delta_pose` labels are the same kind of finite
difference taken over the recorded `EndEffectorPose` observation, which targets the same TCP
the end-effector modes do, so no re-recording is needed there either. See
[Control Modes](/docs/concepts/control-modes) for the layout and the units.
