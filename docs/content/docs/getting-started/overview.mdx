...
MuJoCo is the default simulation backend. It provides all six tasks (PickLift, PickAndPlace, StackCube, Touch, LookAt, Move) on the SO-101 arm, fast single-environment simulation with a live viewer, and custom mesh objects (`MeshObject`). An optional MuJoCo Warp backend (`so101-nexus[warp]`) provides the same six tasks as GPU-parallel batched vector environments for large-scale RL.

## Next Steps

- [End-to-end workflow](/docs/workflow/overview) -- record demonstrations, clone with BC, then reinforce with RL
...
- [Training with PPO](/docs/training/ppo) -- train a baseline policy
