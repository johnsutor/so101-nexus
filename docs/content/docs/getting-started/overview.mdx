...
MuJoCo is the default simulation backend. It provides all five tasks (PickLift, PickAndPlace, Reach, LookAt, Move) on the SO-101 arm, fast single-environment simulation with a live viewer, and custom mesh objects (`MeshObject`). An optional MuJoCo Warp backend (`so101-nexus[warp]`) provides the same five tasks as GPU-parallel batched vector environments for large-scale RL.
