...
Every registered environment is published as a [LeRobot EnvHub](https://huggingface.co/docs/lerobot/en/envhub) package at [johnsutor/so101-nexus-envs](https://huggingface.co/johnsutor/so101-nexus-envs), so a LeRobot user reaches them with one call and no import of this library:
...
The Hub repository holds entry points only: one file per environment id under `envs/`, plus a root `env.py` serving `MuJoCoPickLift-v1`. Each file is a shim over `so101_nexus.envhub.make_env`, which the repository's `requirements.txt` pins, so the physics and reward code is the installed release rather than a copy living on the Hub. Publish an update with `python scripts/publish_envhub.py`.
...
It returns LeRobot's `{env_id: {0: vector_env}}` mapping. Observations use the gym-side convention `lerobot.envs.utils.preprocess_observation` consumes, which is one layer below the `observation.*` keys of `LeRobotEnvWrapper` below:
...
`pixels_agent_pos` withholds `environment_state` on purpose: that is the privileged half of the asymmetric actor-critic split, and a pixels policy must not read it.

The per-episode instruction is exposed as `task_description` on each sub-environment, and success is reported as `info["final_info"]["is_success"]` on the terminating step, which is where LeRobot's rollout looks for both.

Warp ids work the same way, with `device` selecting the simulator device. Their worlds are batched inside one process, so `n_envs` becomes the world count and `use_async_envs` is ignored. The adapter copies observations to host NumPy every step because that is LeRobot's contract; keep the batch on the GPU with `gymnasium.make_vec` when you are training rather than evaluating.
