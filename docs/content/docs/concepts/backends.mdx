...
SO101-Nexus uses MuJoCo as its default simulation backend. A MuJoCo Warp backend adds native GPU-parallel batching for reinforcement learning. Both cover the same six tasks (Touch, LookAt, Move, PickLift, PickAndPlace, StackCube) on the same SO-101 arm, with the same env IDs modulo the `MuJoCo`/`Warp` prefix. See [Environments](/docs/environments) for the ID list.
...
Warp envs produce images through camera observations and do not implement `render()`.
Passing `render_mode` to `gym.make_vec()` produces a warning and has no effect.
For Warp RGB or depth, configure `WristCamera` or `OverheadCamera` in `observations`.
