...
Both backends render camera observations, and observation keys, shapes, and dtypes match. Pixel values do not: the MuJoCo backend rasterizes on CPU through per-env OpenGL renderers, while the MuJoCo Warp backend ray-traces all worlds in one batched pass on the simulation device, returning `uint8` tensors of shape `(num_envs, height, width, 3)`. Warp camera observations are independent of Gymnasium `render_mode`; `render_mode` visualization is MuJoCo-only. See [Backends](/docs/concepts/backends).
