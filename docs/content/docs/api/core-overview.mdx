...
| `SO101_DIR` | `Path` | Asset directory for the SO-101 arm (URDF/XML retained for teleop calibration; both backends load the menagerie MJCF) |
...
| `get_so101_mujoco_model_dir()` | `Path` | Directory of the vendored menagerie MJCF (loaded by both backends) |
| `get_so101_mujoco_model_path()` | `Path` | Path to the menagerie `so101.xml` (loaded by both backends) |
