...
holds the URDF used by the ManiSkill backend and the calibration metadata used
by the teleop tooling.
...
Return the directory and `so101.xml` path of the MJCF model used by the MuJoCo
backend (the vendored MuJoCo Menagerie model under `SO101_menagerie/`). The
MuJoCo backend intentionally diverges from the ManiSkill backend here: MuJoCo
loads this menagerie MJCF, while the ManiSkill URDF and teleop calibration
metadata stay under `SO101/` (see `get_so101_simulation_dir`).
