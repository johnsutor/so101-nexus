...
| `ee_delta_action_scale` | `tuple[float, ...]` | `(0.02, 0.02, 0.02, 0.1, 0.1, 0.1, 0.2)` | Physical scale of a +/-1 `pd_ee_delta_pose` action: `(x, y, z)` in metres, `(wx, wy, wz)` in radians, gripper in radians. Seven positive entries. |
...
object without lifting it) earns no further reward. See
`so101_nexus.rewards.potential_shaping` and `docs/superpowers/skills/
reward-engineering/SKILL.md` in the source repo for the full rationale.
...
When `observations` is not specified: `[JointPositions(), JointVelocities(), EndEffectorPose(), GraspState(), ObjectPose(), ObjectOffset()]` (30 dimensions).
...
object is chosen per episode from an object pool: by default one cube per colour
in `cube_colors`, or pass `objects` to carry `YCBObject` / `MeshObject` instead.

```python
from so101_nexus import PickAndPlaceConfig, YCBObject

# Default cube path (carries a coloured cube onto the disc):
...
When `observations` is not specified: `[JointPositions(), JointVelocities(), EndEffectorPose(), GraspState(), TargetPosition(), ObjectPose(), ObjectOffset(), TargetOffset()]` (36 dimensions).
...
When `observations` is not specified: `[JointPositions(), JointVelocities(), EndEffectorPose(), GraspState(), ObjectPose(), ObjectOffset(), TargetPosition(), TargetOffset()]` (36 dimensions).
...
When `observations` is not specified: `[JointPositions(), JointVelocities(), EndEffectorPose(), GazeDirection()]` (22 dimensions).
