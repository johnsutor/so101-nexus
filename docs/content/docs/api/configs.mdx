...
Ranges apply to every MuJoCo task's side view, including RGB, depth, and the human viewer. Each reset samples the configured ranges using the environment's seeded RNG. The sampled pose stays fixed during the episode; the human viewer still allows manual orbiting. An open viewer adopts the newly sampled pose on reset.

Endpoints must be finite and ordered. Equal endpoints select a fixed value. Omitted angle ranges retain the corresponding fixed angles, and `camera="overhead"` ignores all side ranges. With no ranges, existing behavior is unchanged. Explicit distance bounds can crop the workspace or place the camera inside geometry, so choose bounds that suit your scene. These settings do not change camera observations or add `render()` support to Warp.
