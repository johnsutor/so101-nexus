...
```python
from so101_nexus_core import all_registered_env_ids
...
Returns a list of all SO101-Nexus environment IDs that have been registered with Gymnasium. This includes environments from every backend that has been imported in the current process.

The function returns 7 environment IDs when both backends are imported.
